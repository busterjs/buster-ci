...
function BusterCi(config) {
    ...
    var server = this._server = require('child_process').fork(serverPath);
    ...
    this._killServer = this.killServer.bind(this);
    process.on("close", this._killServer);
...
BusterCi.prototype = {
    ...
    killServer: function(){
        ...
        if (server) {
            ...
            if (this._killServer) {
                process.removeListener("close", this._killServer)
                delete this._killServer;
            }
        ...
        var tasks = {
            ...
            runServer: (function(cb, result){
                ...
                if (server) {
                    ...
                    server.send({
                        method: "run",
                        args: ["-p" + (this._serverCfg.port || 1111)]
                    })
                } else {
                    cb(new Error('server is undefined'))
                }
